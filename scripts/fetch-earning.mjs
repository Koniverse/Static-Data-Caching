...
const webRunnerURL = process.env.WEB_RUNNER_URL || 'https://68dd6977.swwrc.pages.dev/';
...
const runBrowser = async () => {
  ...
  const result = await page.evaluate(async () => {
    try {
      ...
      await koniState.chainService.enableChains(['muse_testnet', 'analog_timechain', 'cere', 'bittensor', 'bittensor_devnet', 'polkadot', 'kusama', 'aleph', 'polkadex', 'ternoa', 'alephTest', 'polkadexTest', 'westend', 'kate', 'edgeware', 'creditcoin', 'vara_network', 'goldberg_testnet', 'moonbeam', 'moonriver', 'moonbase', 'turing', 'turingStaging', 'bifrost', 'bifrost_testnet', 'calamari_test', 'calamari', 'manta_network', 'astar', 'shiden', 'shibuya', 'amplitude', 'amplitude_test', 'kilt', 'kilt_peregrine', 'pendulum', 'bifrost_dot', 'acala', 'parallel', 'interlay', 'krest_network', 'polimec', 'availTuringTest', 'avail_mainnet', 'dentnet']);
